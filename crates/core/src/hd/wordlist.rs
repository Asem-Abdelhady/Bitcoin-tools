//! The 2048 English BIP39 words.
//!
//! Compiled in rather than loaded, because a wallet that cannot find its
//! wordlist is a wallet that cannot read its own backup.
//!
//! Only English. The other eight official lists exist, and adding one is
//! another `&[&str; 2048]` plus a way to say which — but they are not free:
//! the Japanese list is separated by an ideographic space, and every non-ASCII
//! list makes the NFKD normalisation in [`Mnemonic::to_seed`] load-bearing
//! rather than a formality. That work belongs with the feature that asks for
//! it.
//!
//! [`Mnemonic::to_seed`]: crate::hd::Mnemonic::to_seed

/// Every word is unique in its first four letters, which is why a wallet may
/// record a mnemonic truncated to four characters per word.
///
/// Words are three to eight letters, so the shortest ones *are* their own
/// prefix — and those three-letter prefixes are still unique against the
/// four-letter ones, which is the property that makes the truncation safe.
pub const UNIQUE_PREFIX: usize = 4;

/// How many words the list holds — and therefore how many bits one word
/// carries, since 2048 is 2¹¹.
pub const WORD_COUNT: usize = 2048;

/// Bits of entropy one word encodes: `log2(2048)`.
pub const BITS_PER_WORD: usize = 11;

/// The list, in the order BIP39 defines. **The index is the value** — word
/// `n` encodes the 11-bit number `n` — so this array is not merely sorted for
/// lookup, its order is the format.
///
/// It is also sorted lexicographically, which is what lets [`index_of`] be a
/// binary search rather than a scan. Both facts are asserted in this module's
/// tests, along with the published SHA-256 of the list itself.
///
/// A `static` rather than a `const`: a `const` of this size is copied into
/// every place that names it, and there are several.
...
/// The value of `word`, or `None` if it is not in the list.
///
/// A binary search over [`WORDS`], which is sorted. Case-sensitive and
/// untrimmed: BIP39 words are lowercase ASCII and a mnemonic that has been
/// case-folded or padded is a different string, which is
/// [`Mnemonic::parse`](crate::hd::Mnemonic::parse)'s business to tidy before
/// it gets here.
#[must_use]
pub fn index_of(word: &str) -> Option<u16> {
    // The list is 2048 long, so every index fits a u16 — and an 11-bit value
    // is what the caller is actually after.
    WORDS
        .binary_search(&word)
        .ok()
...
/// The word for a value below [`WORD_COUNT`], or `None` above it.
#[must_use]
pub fn word(index: u16) -> Option<&'static str> {
    WORDS.get(usize::from(index)).copied()
}
...
mod tests {
    ...
    /// The list is a protocol constant, so it is pinned to the digest of the
    /// file the BIP publishes — one newline-terminated word per line. A single
    /// transposed or misspelled word changes this, and nothing else in the
    /// suite would notice: a wrong word still round-trips through its own
    /// index, and still produces a seed. It would just be the wrong seed.
    #[test]
    fn is_the_published_list() {
        let file: String = WORDS.iter().map(|w| format!("{w}\n")).collect();
        assert_eq!(
            hex::encode(&sha256(file.as_bytes())),
            "2f5eed53a4727b4bf8880d8f3f199efc90e58503646d9ff8eff3a2ed3b24dbda"
        );
    }

    #[test]
    fn is_sorted_so_the_search_is_valid() {
        for pair in WORDS.windows(2) {
            assert!(pair[0] < pair[1], "{} is not before {}", pair[0], pair[1]);
        }
    }

    /// The property that lets a mnemonic be written four letters per word.
    /// The list holds three-letter words, so a prefix is the first four
    /// letters *or the whole word* — and the truncation is only safe because
    /// those short words do not collide with the truncations either.
    #[test]
    fn every_word_is_unique_in_its_first_four_letters() {
        let mut prefixes: Vec<&str> = WORDS
            .iter()
            .map(|w| &w[..w.len().min(UNIQUE_PREFIX)])
            .collect();
        prefixes.sort_unstable();
        prefixes.dedup();
        assert_eq!(prefixes.len(), WORD_COUNT);
        // Every word is ASCII, so a byte prefix is a character prefix.
        assert!(WORDS.iter().all(|w| w.is_ascii() && w.len() >= 3));
    }

    #[test]
    fn every_word_maps_to_its_own_index() {
        for (i, w) in WORDS.iter().enumerate() {
            let i = u16::try_from(i).expect("2048 indices fit a u16");
            assert_eq!(index_of(w), Some(i), "{w}");
            assert_eq!(word(i), Some(*w));
        }
        assert_eq!(index_of("notaword"), None);
        // Case matters, and so does whitespace.
        assert_eq!(index_of("Abandon"), None);
        assert_eq!(index_of(" abandon"), None);
        assert_eq!(word(2048), None);
    }

    /// 2048 words is exactly eleven bits, which is the arithmetic every
    /// entropy/checksum split in `mnemonic.rs` depends on.
    #[test]
    fn the_list_is_exactly_eleven_bits_wide() {
