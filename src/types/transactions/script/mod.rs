...
/// Lowercase hex, the encoding scripts always appear in.
...
impl Script {
    ...
    pub fn from_hex(hex: &str) -> Result<Self, ParseHexError> {
        if !hex.len().is_multiple_of(2) {
            return Err(ParseHexError::OddLength(hex.len()));
        }
        let mut out = Vec::with_capacity(hex.len() / 2);
        for (i, pair) in hex.as_bytes().chunks(2).enumerate() {
            let s = core::str::from_utf8(pair).map_err(|_| ParseHexError::NotHex(i * 2))?;
            out.push(u8::from_str_radix(s, 16).map_err(|_| ParseHexError::NotHex(i * 2))?);
        }
        Ok(Script(out))
    }
    ...
    pub fn kind(&self) -> ScriptKind {
        ...
        if (b.len() == 35 && b[0] == 0x21 || b.len() == 67 && b[0] == 0x41)
            && b[b.len() - 1] == all::OP_CHECKSIG.to_u8()
        {
        ...
        if let Some(kind) = self.witness_program() {
            return kind;
        }
    ...
    pub fn fields(&self) -> ScriptFields {
        ...
        match self.kind() {
            ScriptKind::P2PK => ScriptFields::P2Pk {
                pubkey: to_hex(&b[1..b.len() - 1]),
            },
            ScriptKind::P2PKH => ScriptFields::P2Pkh {
                pubkey_hash: to_hex(&b[3..23]),
            },
            ScriptKind::P2SH => ScriptFields::P2Sh {
                script_hash: to_hex(&b[2..22]),
            },
            ScriptKind::P2WPKH
            | ScriptKind::P2WSH
            | ScriptKind::P2TR
            | ScriptKind::WitnessUnknown { .. } => ScriptFields::Witness {
                witness_version: if b[0] == 0 { 0 } else { b[0] - 0x50 },
                witness_program: to_hex(&b[2..]),
            },
            ScriptKind::P2MS => {
                ...
                ScriptFields::P2Ms {
                    ...
                    pubkeys: steps[1..steps.len() - 2]
                        .iter()
                        .filter_map(|s| s.instruction.data().map(to_hex))
                        .collect(),
                }
            }
            ScriptKind::OpReturn => {
    ...
    fn witness_program(&self) -> Option<ScriptKind> {
        let b = &self.0;
        if b.len() < 4 || b.len() > 42 {
            return None;
        }
    ...
    /// `OP_m <pubkey>{n} OP_n OP_CHECKMULTISIG`, with 1 <= m <= n <= 3
    /// for the bare (non-P2SH) form that relay policy accepts.
...
impl fmt::Display for ParseHexError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ParseHexError::OddLength(n) => write!(f, "hex string has odd length {n}"),
            ParseHexError::NotHex(i) => write!(f, "invalid hex at byte {i}"),
        }
    }
}
...
mod tests {
    ...
    fn classifies_standard_outputs() {
        // Taken from the vectors in src/examples/.
        assert_eq!(
    ...
    #[test]
    fn hex_errors() {
        assert_eq!(Script::from_hex("abc"), Err(ParseHexError::OddLength(3)));
        assert_eq!(Script::from_hex("zz"), Err(ParseHexError::NotHex(0)));
    }
}
